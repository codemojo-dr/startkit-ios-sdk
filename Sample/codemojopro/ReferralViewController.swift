...
class ReferralViewController: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        log.getReferral(userid, Access_token: appDelegate.Access_token, EnviromentType: appDelegate.env_type) {[weak self] (status, code, url, friend_earn, you_earn) in
            ...
            dispatch_async(dispatch_get_main_queue(), { 
                ...
                self?.appDelegate.referralurl = "\(url)"
                
                self?.appDelegate.referralcode = "\(code)"
    ...
    @IBAction func usereferralact(sender: AnyObject) {
        ...
        if refcode != "" {
            ...
            log.UseReferral(userid, referralcode: refcode!, Access_token: appDelegate.Access_token, EnviromentType: appDelegate.env_type, completion: { (status, message) in
...
class MyStringItemSource: NSObject, UIActivityItemSource {
    ...
    @objc func activityViewController(activityViewController: UIActivityViewController, itemForActivityType activityType: String) -> AnyObject? {
        ...
        return "Hi there, I have been using Codemojo Demo and fiund it very useful. \n Would be great if you can join me by clicking \(self.appDelegate.referralurl) or use the cde \(self.appDelegate.referralcode)"
    }
